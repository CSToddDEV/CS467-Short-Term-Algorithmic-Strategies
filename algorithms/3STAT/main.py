...
class FirstAlgo(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        for resolution in self.resolutions:
            ...
            # Sell Signal
            elif bar.Close < self.moving_avg_close[self._equity][resolution].Current.Value and \
    ...
    def set_universe(self):
        ...
        for ticker in self.equities:
            ...
            # Fill out short list
                ...
                while i < 3 and i < len(top_volatility) and (ticker in self.bb.keys()) and not inserted:
                    if self.bb[ticker].StandardDeviation.Current.Value >= self.bb[
                        top_volatility[i]].StandardDeviation.Current.Value:
        ...
        if (self.moving_avg[top][3].Current.Value - self.bb[top].LowerBand.Current.Value) > (
                self.moving_avg[top_volatility[1]][3].Current.Value - self.bb[
            top_volatility[1]].LowerBand.Current.Value):
            top = top_volatility[1]
        if (self.moving_avg[top][3].Current.Value - self.bb[top].LowerBand.Current.Value) > (
                self.moving_avg[top_volatility[2]][3].Current.Value - self.bb[
