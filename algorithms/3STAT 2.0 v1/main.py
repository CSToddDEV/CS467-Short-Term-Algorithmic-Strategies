...
class FirstAlgo(QCAlgorithm):
    def Initialize(self):
        ...
        self.SetEndDate(2021, 12, 30)  # Set End Date
    ...
    def set_universe(self):
        ...
        # Check Momentum Average, set milepost for keeping same equity
        if positive_tickers != 0:
            avg_mom = avg_mom / positive_tickers
            self.Debug("MOMENTUM AVERAGE: {0}".format(avg_mom))
            if avg_mom > 0:
                avg_mom = 5
            else:
                avg_mom = -5

        # Check to see if we keep same equity
        if self._equity is not None and self.mom[self._equity].Current.Value > avg_mom:
            return
        ...
        # Lowest Volatility is our New Universe selection
        top = low_v[0]
